...
CREATE OR REPLACE PACKAGE BODY PLATFORMADMIN_CTX_PKG IS
    -- Define the procedure to set the platform administrator context
    PROCEDURE SET_PLATFORMADMIN_CONTEXT IS
        ADMIN_ID NUMBER;
    BEGIN
        -- Retrieve the AdminID based on the current session user
        SELECT AdminID INTO ADMIN_ID
        FROM PlatformAdmins
        WHERE Username = SYS_CONTEXT('USERENV', 'SESSION_USER');

        -- Set the admin_id in the platformadmin_ctx context
